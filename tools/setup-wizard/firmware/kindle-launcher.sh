...
###############################################################################
# PTV-TRMNL Kindle Launcher Script
# Launches PTV-TRMNL display on jailbroken Kindle devices
#
...
# Configuration
SERVER_URL="YOUR_RENDER_URL"  # e.g., https://ptv-trmnl-yourname.onrender.com
DEVICE_TYPE="kindle-pw3"      # Or kindle-pw4, kindle-pw5, kindle-4
REFRESH_INTERVAL=300          # 5 minutes (300 seconds) minimum for Kindle

# Paths
LOG_FILE="/mnt/us/extensions/ptv-trmnl/ptv-trmnl.log"
PID_FILE="/var/run/ptv-trmnl.pid"
CACHE_DIR="/mnt/us/extensions/ptv-trmnl/cache"
...
if [ -f "$PID_FILE" ]; then
    ...
    if kill -0 "$PID" 2>/dev/null; then
        log "PTV-TRMNL is already running (PID: $PID)"
        exit 0
    else
...
log "=== PTV-TRMNL Kindle Launcher Starting ==="
